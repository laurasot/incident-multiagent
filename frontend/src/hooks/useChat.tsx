...
export function ChatProvider({ children }: ChatProviderProps) {
      ...
      try {
        for await (const event of invokeAgentStream(
          ...
          if ('event' in event && event.event && typeof event.event === 'object') {
            ...
            // Handle transfer_to_agent action within nested event (plantilla_cdk / Strands format)
            if ('actions' in innerEvent && typeof innerEvent.actions === 'object' && innerEvent.actions !== null) {
              const actions = innerEvent.actions as any;
              if (actions.transfer_to_agent && typeof actions.transfer_to_agent === 'string') {
                ...
                setChatState((prev) => ({ ...prev, activeAgent: actions.transfer_to_agent }));
              }
            }
          }
          ...
          // Handle Strands events: {type: "text", content: "..."}, {type: "tool_use", ...}, {type: "tool_result", ...}
            ...
            if (eventType === 'text') {
              ...
              const resultData = (event as any).content;
              if (toolName) {
                const existing = toolBlocks.get(toolName);
                if (existing) {
                  const resultText = resultData
                    ? (Array.isArray(resultData)
                        ? resultData.map((c: any) => c.text || c.json || JSON.stringify(c)).join('\n')
                        : typeof resultData === 'string'
                          ? resultData
                          : JSON.stringify(resultData))
                    : '';
                  toolBlocks.set(toolName, {
                    ...existing,
                    result: resultText,
                    status: 'success',
                  });
  ...
  const initializeConversation = useCallback(
    async (bearerToken: string, actorId: string) => {
      const defaultPrompt = `Hola, ¿cómo estás?`;
